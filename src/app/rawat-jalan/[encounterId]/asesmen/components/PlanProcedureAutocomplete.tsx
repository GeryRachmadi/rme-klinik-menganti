...
interface PlanProcedureAutocompleteProps {
  onProcedureChange: (procedure: SelectedProcedure) => void;
}

export function PlanProcedureAutocomplete({
  onProcedureChange,
}: PlanProcedureAutocompleteProps) {
  ...
  const { results, isLoading } = useProcedureSearch(searchQuery);
  ...
  const handleAddManual = () => {
    ...
    onProcedureChange({ code: 'MANUAL', display: sanitized, category: 'Manual', notes: manualNote.trim() });
    setManualText('');
    setManualNote('');
  ...
  // ── Manual mode (mirrors AssessmentDiagnosisForm pattern) ────────────────
  if (isManualMode) {
